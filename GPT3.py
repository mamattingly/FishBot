...
# NEW ➤ Adjustable bite-detection timeout (seconds)
BITE_TIMEOUT = 15
...
# ---------------------------------------------------------
# BITE DETECTION (WITH TIMEOUT)
# ---------------------------------------------------------
def detect_bite(bx, by, threshold=25000):
    """
    Improved bite detection:
    ✔ 500ms stabilization delay after cursor movement
    ✔ ignore first 8 warmup frames
    ✔ must detect motion for 3 consecutive frames
    ✔ timeout safe-guards
    """
    ...
    with mss.mss() as sct:
        ...
        # -----------------------------------------------------
        # 1. Stabilization delay (ignore mouse movement noise)
        # -----------------------------------------------------
        time.sleep(0.5)

        # -----------------------------------------------------
        # 2. Warm-up frames (ignore initial noise)
        # -----------------------------------------------------
        prev_gray = None
        for _ in range(8):
        ...
        # -----------------------------------------------------
        # 3. Real motion detection loop
        # -----------------------------------------------------
        motion_frames = 0
        required_frames = 3  # must detect motion multiple times in a row

        while running:

            # Timeout check
            if time.time() - start_time > BITE_TIMEOUT:
                print(f"⏰ No motion detected after {BITE_TIMEOUT} seconds. Recasting...")
                return False
            ...
            if score > threshold:
                ...
                motion_frames = 0  # reset if motion not consistent

            # Confirmed motion across multiple frames
...
def fish():
    ...
    while running:
        ...
        if not bobber:
            print("❌ Bobber not found after casting. Recasting...")
            continue
        ...
        # UPDATED: detect_bite() returns False if timeout occurs
